...
class CacheTests: XCTestCase {
    ...
    var cache = Cache(name: "CacheTests")
    ...
    override func tearDown() {
        ...
        guard let diskCache = AEPServiceProvider.shared.cacheService as? DiskCacheService else { return }
        let cachePath = diskCache.cachePath(for: "CacheTests")
        try! FileManager.default.removeItem(atPath: cachePath)
    }
    
    /// When an entry doesn't exist in the cache we should return nil
    func testGetEmpty() {
        XCTAssertNil(cache.get(key: ENTRY_KEY))
    }
    ...
    func testSetGetHappy() {
        ...
        // test
        try! cache.set(key: ENTRY_KEY, entry: entry)
        
        // verify
        let storedEntry = cache.get(key: ENTRY_KEY)
    ...
    func testSetGetHappyMany() {
        ...
        for i in 0..<count {
            ...
            try! cache.set(key: "\(i)", entry: entry)
        }
        
        // verify
        for i in 0..<count {
            let storedEntry = cache.get(key: "\(i)")
    ...
    func testSetShouldOverwrite() {
        ...
        // test
        try! cache.set(key: ENTRY_KEY, entry: entry)
        try! cache.set(key: ENTRY_KEY, entry: newEntry)
    ...
    func testRemoveInvalidThrows() {
        XCTAssertThrowsError(try cache.remove(key: ENTRY_KEY))
    }
    ...
    func testSetGetRemoveGet() {
        ...
        // test pt. 2
        try! cache.remove(key: ENTRY_KEY)
