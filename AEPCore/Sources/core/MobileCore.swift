...
public final class MobileCore: NSObject {
    static let LOG_TAG = "MobileCore"
    
    /// A shared instance of `WrapperTypeProvider` for managing and retrieving wrapper type information.
    /// Ensures thread-safe access to wrapper type data across different `EventHub` instances.
    static let wrapperTypeProvider = WrapperTypeProvider()
    
    /// This will be used to create and retrieve event hubs for different SDK instances.
    static let eventHubProvider = EventHubProvider(wrapperTypeProvider: wrapperTypeProvider)

    /// A dispatch queue for thread-safe access to the `apiStore`.
    static let apiQueue = DispatchQueue(label: "com.adobe.apiStore.access")
    ...
    @objc public static var extensionVersion: String {
        let wrapperType = wrapperTypeProvider.wrapperType
        if wrapperType == .none {
    ...
    public static func setWrapperType(_ type: WrapperType) {
        wrapperTypeProvider.wrapperType = type
    }
    ...
    public static func registerExtensions(_ extensions: [NSObject.Type], _ completion: (() -> Void)? = nil) {
        apiDefaultInstance.registerExtensions(extensions, completion)        
    }
    ...
    public static func registerExtension(_ exten: Extension.Type, _ completion: (() -> Void)? = nil) {
        apiDefaultInstance.registerExtension(exten, completion)
    }
    ...
    public static func unregisterExtension(_ exten: Extension.Type, _ completion: (() -> Void)? = nil) {
        apiDefaultInstance.unregisterExtension(exten, completion)
    }
    ...
    public static func getRegisteredExtensions() -> String {
        apiDefaultInstance.getRegisteredExtensions()
    }
    ...
    public static func dispatch(event: Event) {
        apiDefaultInstance.dispatch(event: event)
    }
    ...
    public static func dispatch(event: Event, timeout: TimeInterval = 1, responseCallback: @escaping (Event?) -> Void) {
        apiDefaultInstance.dispatch(event: event, timeout: timeout, responseCallback: responseCallback)
    }
    ...
    public static func registerEventListener(type: String, source: String, listener: @escaping EventListener) {
        apiDefaultInstance.registerEventListener(type: type, source: source, listener: listener)
    }
    ...
    public static func setAdvertisingIdentifier(_ identifier: String?) {
        apiDefaultInstance.setAdvertisingIdentifier(identifier)
    }
    ...
    public static func setPushIdentifier(_ deviceToken: Data?) {
        apiDefaultInstance.setPushIdentifier(deviceToken)
    }
    ...
    public static func collectMessageInfo(_ messageInfo: [String: Any]) {
        apiDefaultInstance.collectMessageInfo(messageInfo)
    }
    ...
    public static func collectLaunchInfo(_ userInfo: [String: Any]) {
        apiDefaultInstance.collectLaunchInfo(userInfo)
    }
    ...
    public static func collectPii(_ data: [String: Any]) {
        apiDefaultInstance.collectPii(data)
    }
}
