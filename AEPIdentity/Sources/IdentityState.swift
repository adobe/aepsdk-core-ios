...
class IdentityState {
    ...
    func syncIdentifiers(event: Event, forceSync: Bool = false) -> [String: Any]? {
        ...
        // check is this is forceSync event. Currently the checks for event.forceSync is not very useful as there no events with that property set. It still here to support that workflow when there is a way to update the eventData.
