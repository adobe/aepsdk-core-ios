...
class IdentityPropertiesTests: XCTestCase {
    ...
    func testJSON() {
        let json = """
        {
          ...
          "ttl": 30,
          "ecid": {
            "ecidString": "12057873598405548475607764679698953852"
          }
        }
        """.data(using: .utf8)!
        
        let props = try! JSONDecoder().decode(IdentityProperties.self, from: json)
    ...
    func test2() {
        ...
        let props = try! JSONDecoder().decode(IdentityProperties.self, from: identityPropsData)
