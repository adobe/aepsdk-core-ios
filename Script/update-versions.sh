...
if [ "$DEPENDENCIES" != "none" ]; then
    ...
    for dependency in "${dependenciesArray[@]}"; do
        ...
        if [ "$spmRepoUrl" != "" ]; then
            ...
            $SED_COMMAND "/$spmRepoUrl\", \.upToNextMajor/{s/$VERSION_REGEX/$dependencyVersion/;}" $SPM_FILE
        fi
    done
fi
...
if [ "$NAME" == "Services" ]; then
    ...
    $SED_COMMAND "/^ +static let EXTENSION_VERSION/{s/$VERSION_REGEX/$NEW_VERSION/;}" $CONSTANTS_FILE
    ...
    $SED_COMMAND "/^ +static let VERSION_NUMBER/{s/$VERSION_REGEX/$NEW_VERSION/;}" $EVENT_HUB_FILE
else
...
if [ "$NAME" == "Core" ]; then
    ...
    $SED_COMMAND "/^ +\"version\" : \"/{s/[1-9]+\.[0-9]+\.[0-9]+/$NEW_VERSION/;}" $TEST_FILE
fi
...
$SED_COMMAND "/^\t+MARKETING_VERSION = /{s/$VERSION_REGEX/$NEW_VERSION/;}" $PROJECT_PBX_FILE
